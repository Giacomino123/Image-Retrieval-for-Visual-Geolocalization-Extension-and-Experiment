...
if args.grl:
    dataset_root = f"{args.datasets_folder}/{args.dataset_name}/images"
    grl_datasets = f"{dataset_root}/target + {dataset_root}/test/queries"
    grl_dataset = grl.GrlDataset(dataset_root, args.grl_datasets.split("+"))
...
if args.grl:
  ...
  grl_discriminator = grl.get_discriminator(args.encoder_dim, len(args.grl_datasets.split("+")))
else:
  grl_discriminator = None
model = network.GeoLocalizationNet(args, grl_discriminator)
netvladlayer = network.get_aggregation(args)
...
#### Setup Optimizer and Loss and Scheduler
if args.aggregation == "crn":
    ...
    if args.optim == "adam":
        ...
        optimizer = torch.optim.SGD([{'params': crn_params, 'lr': args.lr_crn_layer, 'momentum': 0.9, 'weight_decay': args.wd},
                                      {'params': net_params, 'lr': args.lr_crn_net, 'momentum': 0.9, 'weight_decay': args.wd}])
else:
    if args.optim == "adam":
...
if args.scheduler:
  if args.scheduler == "ReduceLROnPlateau":
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min",
            factor=args.factor, patience=2, verbose=True, min_lr=1e-8, threshold=args.threshold, cooldown=2, threshold_mode='abs')
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    if args.grl:
        ...
        grl_dataloader = DataLoader(dataset=grl_dataset, num_workers=args.num_workers,
                                    batch_size=args.grl_batch_size, shuffle=True, pin_memory=args.device)
        iteration = iter(grl_dataloader)
    ...
    for loop_num in range(loops_num):
        ...
        if args.scheduler == "ReduceLROnPlateau":
          ...
          logging.info(f"Learning rate: {optimizer.state_dict()['param_groups'][0]['lr']}")
        elif args.scheduler == "CosineAnnealingLR":
          scheduler.step()
          logging.info(f"Learning rate: {optimizer.state_dict()['param_groups'][0]['lr']}")          
