    ...
    with torch.no_grad():
        ...
        for inputs, indices in tqdm(queries_dataloader, ncols=100):
            ...
            if args.multi_scale and args.multi_scale_method == 'avg':
                ...
                features = torch.mean(feature.type(torch.float32), dim=-1)
                ...
                for i, j in zip(H_list, W_list):
                    ...
                    multi_scale.append(feature_ms)
                feature = torch.stack(multi_scale, -1)
                features = torch.sum(feature.type(torch.float32), dim=-1)
                ...
                for i, j in zip(H_list, W_list):
                    ...
                    multi_scale.append(feature_ms)
                feature = torch.stack(multi_scale, -1)
                features, max_index = torch.max(feature.type(torch.float32), dim=-1)
                del max_index
                ...
                for i, j in zip(H_list, W_list):
                    ...
                    multi_scale.append(feature_ms)
                feature = torch.stack(multi_scale, -1)
                features, min_index = torch.min(feature.type(torch.float32), dim=-1)
                del min_index
            #    
            else:    
                features = model(inputs.to(args.device))
