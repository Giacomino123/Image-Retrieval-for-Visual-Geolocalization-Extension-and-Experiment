...
import torchvision.transforms as transforms         # torchvision.transforms.ColorJitter ---- torchvision.transforms.functional.adjust_brightness
...
class BaseDataset(data.Dataset):                       
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        self.select_resolutions = args.select_resolutions
        self.multi_scale_method = args.multi_scale_method
...
class TripletsDataset(BaseDataset):        
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        # initialization for smart data augmentation
        self.night_brightness = args.night_brightness
        self.night_hue = args.night_hue
        self.night_contrast = args.night_contrast
        self.night_saturation = args.night_saturation
    ...
    def __getitem__(self, index):
        ...
        query     =  path_to_pil_img(self.queries_paths[query_index])
        
        # implementation for smart data augmentation
        if self.night_brightness != None :
            query = transforms.functional.adjust_brightness(query, brightness_factor=self.night_brightness)
        if self.night_hue != None :
            query = transforms.functional.adjust_hue(query, hue_factor=self.night_hue)
        if self.night_contrast != None :
            query = transforms.functional.adjust_contrast(query, contrast_factor=self.night_contrast)
        if self.night_saturation != None :
            query = transforms.functional.adjust_saturation(query, saturation_factor=self.night_saturation)
        query = self.query_transform(query)
    
        positive  =  self.resized_transform(path_to_pil_img(self.database_paths[best_positive_index]))
        # if self.night_brightness != None :
        #     positive = transforms.functional.adjust_brightness(positive, brightness_factor=self.night_brightness)
        
        negatives = [self.resized_transform(path_to_pil_img(self.database_paths[i])) for i in neg_indexes]
        # if self.night_brightness != None :
        #     transf_neg = []
        #     for neg in negatives:
        #         transf_neg.append(transforms.functional.adjust_brightness(neg, brightness_factor=self.night_brightness))
            
        # if self.night_brightness != None :
        #     images = torch.stack((query, positive, *transf_neg), 0)
        # else:
